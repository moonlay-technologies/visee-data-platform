...
def getTimeFilter(ti, **kwargs):
    ...
    filter_start = (formated_times - timedelta(minutes=55)).replace(second=0, microsecond=1)
...
# ------------------Truncate Raw Data-------------------
viseetor_line_truncate = PostgresOperator(
    task_id='viseetor_line_truncate',
    postgres_conn_id='postgres_visee',
    sql='sql/truncate_viseetor_line.sql',
    dag=dag
)


# ---------------------------DAG Flow----------------------------
get_time_filter >> get_data_dynamodb >> raw_line_to_live_visitor >> viseetor_line_truncate